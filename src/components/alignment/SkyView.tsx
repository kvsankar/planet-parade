...
export default function SkyView({ bodies, date, center, onCenterChange }: SkyViewProps) {
  ...
  useEffect(() => {
    ...
    const obs = new ResizeObserver((entries) => {
      ...
      if (entry) setWidth(Math.max(400, Math.floor(entry.contentRect.width)))
    })
    obs.observe(el)
  ...
  const { spanInfos, morningRects, eveningRects } = useMemo(() => {
    ...
    return { spanInfos: spans, morningRects: buildShadeRects(mPlanets), eveningRects: buildShadeRects(ePlanets) }
  }, [plotData, refLon])
  ...
  // Layout — extra bottom space for span annotations
  ...
  const height = Math.max(200, Math.round(plotW * 0.18) + spanAreaH)
  ...
  return (
    <div className="sky-view" ref={containerRef}>
      ...
      >
        ...
        {/* Morning sub-range shading (west of Sun) */}
        <g clipPath={`url(#${clipId})`}>
          {morningRects.map((r, i) => {
            const rx = xOf(r.left)
          ...
          {/* Evening sub-range shading (east of Sun) */}
          {eveningRects.map((r, i) => {
            const rx = xOf(r.left)
            const rw = xOf(r.right) - rx
            return (
              <g key={`pm${i}`}>
